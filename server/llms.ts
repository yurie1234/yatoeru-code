...
> 登録支援機関（特定技能）・監理支援機関（育成就労）を比較できるデータベース。出入国在留管理庁の公開登録簿（一次情報）を基に掲載し、対応言語・地域・業種・行政処分歴・新規相談受付状況で検索できます。事業者への直接確認情報を順次反映しており、事業者への確認が取れた機関には「掲載情報 運営確認済み」と確認日を表示しています。相談・掲載情報の確認修正はすべて無料です。
