...
// マネタイズ設計の原則（2026-07-16 確定）：
// 1. 評価と有料の分離 —— 診断（準備度チェック）の推奨機関は条件ロジックのみで決定し、
//    順位・推奨枠を販売しない。有料枠を将来置く場合は診断結果ページ下部に
//    「広告」表記つきの別枠として完全分離する（ステマ規制・景表法対策）。
// 2. PR枠の運用ルール —— PR表示は各ページ上位2件まで。オーガニックの並び順
//    （親和性スコア等）には一切影響しない。PRバッジの明示は必須。
// 3. 口コミは封印中 —— 投稿ガイドライン・本人確認・削除基準・法務体制の整備後（GO判定後）に公開。
// 4. 「専用ランディングページ作成」はテンプレートからの自動生成である限り提供する
//    （手作業の受託労働を混入させない。属人性ゼロ原則）。
const PLANS = [
  {
    key: "free" as const,
    name: "無料掲載",
  ...
  },
  {
    ...
    features: [
      "スタンダードのすべて",
      "地域×分野ページのPR枠での表示（PRバッジ明示）",
      "専用ランディングページ作成（テンプレート自動生成）",
      "四半期ごとの市場レポート提供",
    ],
    cta: "プレミアムで申し込む",
    highlight: true,
  ...
  return (
    <SiteLayout>
      <div className="bg-brand text-brand-foreground py-16">
        <div className="container max-w-3xl text-center">
          ...
          <p className="text-brand-foreground/70 leading-relaxed">
            ヤトエルには準備度チェックを通じて、受入意欲の高い企業からの相談が集まります。<br />
      ...
      <div className="container py-16">
        ...
        {/* PR枠の運用ルール（公開明文化） */}
        <div className="max-w-5xl mx-auto mb-16 -mt-8">
          <p className="text-xs text-muted-foreground leading-relaxed rounded-md border bg-muted/40 px-4 py-3">
            【PR掲載の運用ルール】PR枠は各ページ上位2件までとし、必ずPRバッジを明示します。
            検索結果の並び順（親和性スコア等）や準備度チェックの推奨機関の選定に、有料プランの契約状況は一切影響しません。
            推奨機関は分野・地域・対応言語等の条件ロジックのみで決定されます。
          </p>
        </div>
